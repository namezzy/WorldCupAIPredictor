...
const scoringRules = [
  {
    ...
    points: "+3 points",
    description: "Predict the final scoreline perfectly and bank the full reward.",
  },
  {
    title: "Correct outcome",
    points: "+1 point",
    description: "Pick the right winner or draw even if the exact score is off.",
  },
  {
    title: "Wrong prediction",
    points: "0 points",
    description: "Miss both the score and match outcome, and the round stays empty.",
  },
];
...
export default function LeaderboardPage() {
  ...
  const totalPredictions = mockLeaderboard.reduce(
    (total, entry) => total + entry.total_predictions,
    0
  );
  const highestScore = mockLeaderboard[0]?.total_points ?? 0;
    ...
    <div className="container mx-auto space-y-10 px-4 py-8">
      <section className="space-y-4">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2 text-sm text-muted-foreground">
            <Badge variant="outline" className="border-white/10 bg-background/60">Mock season · 12 matches</Badge>
            <Badge variant="outline" className="border-brand-gold/30 bg-brand-gold/10 text-brand-gold">Current player highlighted</Badge>
          </div>
        </div>
      </section>
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-display text-2xl font-bold">Podium leaders</h2>
        </div>
        <Podium entries={topThree} />
      </section>

      <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px] xl:items-start">
        <RankingTable entries={mockLeaderboard} currentUserId={mockCurrentUserId} />
